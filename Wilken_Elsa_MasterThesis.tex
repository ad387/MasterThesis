...
\section{Tungurahua}
	...
	6.4\% of the evaluated volcanic gas plumes contained \ce{SO2} above the "plume threshold of $7\cdot10^{17}\quad \frac{molec}{cm^2}$. Thus 6.4\% of the data can be used for the examination of the volcanic gas emissions in this timespan.\\
 	...
 	A higher amount of spectra within the \ce{SO2}  plume limit can be found in the contaminated data even if the contaminated data are evaluated with the NOVAC-method. Here, the percentage of data in the plume-limit is 0.153\%.
...
At Nevado Del Ruiz a larger time span (from the end of 2009 to the end of 2011) is examined. Thus, there is a higher amount of "Multi Add" data of 14005.
...
The total amount of contamination data is 1392. This is equivalent to 9.9\% of all data. Evaluating the contaminated data with the NOVAC-method in 32.8\% of the cases yields SO2 SCDs within the plume limit. of the \ce{SO2} SCDs within the plume limit. As at Tungurahua the occurrence of data above the plume limit within the contaminated data is larger as for not contaminated data (by a factor of 2.7).\\
The following paragraph only deals with the contaminated data.
If using the contamination based method 0.598\% of the \ce{SO2} column densities are above the detection limit. Thus, an increase of \ce{SO2} SCDs above the plume limit within the contaminated data of 95\% is observable. In total the amount of \ce{SO2} SCDs above the plume limit increases by 2.7 percent points. As a result 14.8\% of all data are above the plume limit.\\
\FloatBarrier
...
%	\Cref{fig:tungso2novacsametime} shows the equivalent plot for the Tungurahua volcano.\\
	...
	\ce{BrO}  column densities above the detection limit often the daily mean of the \ce{BrO}/\ce{SO2}  ratios is used. Hereby at least four "Multi Adds" per day in the plume limit are considered in order to avoid outliers. Thus, performing the evaluation of contaminated data with the contamination based method leads to more data.\textcolor{red}{wie gehts eigentlich dem mitteleren BrO/SO2 fehler? (also (BrO/SO2)/sqrt(datenpunkte an dem tag) )}
	%Some days which had less than  three to four valid data points could then have four or more multi adds. 
	At Tungurahua 30.8\% more daily mean data can be retrieved when using the contamination based method compared to exclude the contaminated data. The amount of daily means increases less than the total amount of data, this effect can be explained due to a higher occurence of contamination if the \ce{SO2}  column densities are high, thus more data are retrieved for days with a high \ce{SO2}  amount.
